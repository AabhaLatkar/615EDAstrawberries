...
#\| label: split srawberry into census and survey pieces #\| echo: false
...
plot3 <- strwb_census |> 
  filter(Totals == "PRODUCTION")|> select(c(Year, Value))
# Create a line graph
g <- ggplot(data = plot3, aes(x = Year, y = Value)) +
  geom_point() +
...
strwb_survey <- strwb_survey %>%
  rename(Fresh = `Fresh Market`, Process = `Process Market`)
plot3 <- strwb_census |> 
  filter((Totals == "PRODUCTION"), (Fresh == "PRODUCTION"), (Process == "PRODUCTION"))|> select(c(Year, Value))
# Create a line graph
g <- ggplot(data = plot3, aes(x = Year, y = Value))+
  geom_line() +
  labs(x = "Years", y = "Value") +
